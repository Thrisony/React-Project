import React, { useState, useEffect, useRef } from "react";
import "../Student/Sidenav1.css"; // Include your styles here
import n1 from '../Student/n1.png';
import n2 from '../Student/n2.png';
import n3 from '../Student/n3.png';
import n4 from '../Student/n4.png';

import Sidenav from "./Sidenav";
export const Mainadmin1 = () => {
  const [active, setActive] = useState(0);
  const listRef = useRef(null);
  const items = [n4, n2, n3, n1];

  const lengthItems = items.length - 1;

  useEffect(() => {
    const autoSlide = setInterval(() => {
      handleNext();
    }, 3000);

    return () => clearInterval(autoSlide);
  }, [active]);

  const handleNext = () => {
    setActive((prev) => (prev + 1 > lengthItems ? 0 : prev + 1));
  };

  const handlePrev = () => {
    setActive((prev) => (prev - 1 < 0 ? lengthItems : prev - 1));
  };

  const handleDotClick = (index) => {
    setActive(index);
  };

  useEffect(() => {
    if (listRef.current) {
      const checkLeft = listRef.current.children[active].offsetLeft;
      listRef.current.style.left = `-${checkLeft}px`;
    }
  }, [active]);

  return (
    <div className="app-container">
        <Sidenav/>
      <header className="hero1">
        <div className="slider">
          <div className="list" ref={listRef}>
            {items.map((src, index) => (
              <div className="item" key={index}>
                <img src={src} alt={`image${index + 1}`} />
              </div>
            ))}
          </div>
          <div className="buttons">
            <button id="prev" onClick={handlePrev}>
              &lt;
            </button>
            <button id="next" onClick={handleNext}>
              &gt;
            </button>
          </div>
          <ul className="dots">
            {items.map((_, index) => (
              <li
                key={index}
                className={index === active ? "active" : ""}
                onClick={() => handleDotClick(index)}
              ></li>
            ))}
          </ul>
        </div>
        <div className="hero-text">
          <h1>Welcome to NEC Bus Pass Management System</h1>
          <p>Digital Pass, Seamless Journey – Revolutionizing Bus Pass Management</p>
          <div className="hero-buttons">
            <button className="btn btn-primary">Admin</button>
            <button className="btn btn-secondary">Features</button>
          </div>
        </div>
      </header>

 {/* Features Section */}
<section className="features">
  <h2>Admin Panel Features</h2>
  <p>
    The admin panel streamlines the bus pass management process, offering robust controls and efficient workflows.
  </p>
  <div className="features-grid">
    <div className="feature-item">
      <div className="feature-icon">🔑</div>
      <h3>Verify Applications</h3>
      <p>Review and verify student bus pass applications with ease.</p>
    </div>
    <div className="feature-item">
      <div className="feature-icon">🚌</div>
      <h3>Manage Buses and Routes</h3>
      <p>Add, update, or remove bus routes and manage bus capacities efficiently.</p>
    </div>
    <div className="feature-item">
      <div className="feature-icon">🎫</div>
      <h3>Generate Bus Pass</h3>
      <p>Generate and issue digital bus passes for approved applications.</p>
    </div>
    <div className="feature-item">
      <div className="feature-icon">📊</div>
      <h3>Track and Monitor</h3>
      <p>Track the overall bus pass issuance process and application statuses.</p>
    </div>
    <div className="feature-item">
      <div className="feature-icon">📧</div>
      <h3>Send Notifications</h3>
      <p>Send alerts and notifications to students about application updates and approvals.</p>
    </div>
    <div className="feature-item">
      <div className="feature-icon">🔒</div>
      <h3>Secure Access</h3>
      <p>Maintain a secure system with role-based access controls for data safety.</p>
    </div>
  </div>
</section>



    </div>
  );
};
